...
@section('content')
    ...
    <div class="row div-content">
        <div class="col-12 pt-4 pb-3">
            <h1><a href="{{ route('discussion.index') }}" style="color: rgb(154, 164, 170);"><i class="fas fa-angle-left"></i></a>  修改文章</h1>
        </div>
        <form class="col-12" action="{{ route('discussion.update',['discussion'=>$discussion]) }}" method="post">
            @csrf
            @method('PATCH')
            <div class="row">
                <div class="col-6">
                    <div class="item">
                        ...
                        <div class="item_text">
                            <select class="form-control" id="item" name="type">
                                @foreach($type as $t)
                                    @if($t->type != '欣賞修學報告')
                                        <option value="{{ $t->id }}" {{ $discussion->type == $t->id ? "selected":""}}>{{ $t->type }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="col-12 pt-3">
                    <textarea name="content">{!! $discussion->content !!}</textarea>
                </div>
            </div>
        </form>
    </div>
