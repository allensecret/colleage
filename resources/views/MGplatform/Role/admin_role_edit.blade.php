...
@section('navbar')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item text_label"><b>更新角色與權限</b></li>
    </ol>
@endsection
@section('content')
    <div class="content-wrapper">
        <div class="container-fluid">
            <form class="row" action="{{ route('role.update',['role'=>$role]) }}" method="post">
                @method('PATCH')
                @csrf
                <div class="col-8">
                ...
                <div class="col-4">
                    <button type="submit" class="btn btn-primary" style="float: right"><i class="fas fa-plus"></i>更新</button>
                </div>
                <div class="col-12 pt-3">
                    <div class="form-group">
                        <label for="Character" style="font-size: 1.3rem">角色名稱：</label>
                        <input type="text" class="form-control" id="Character" name="Character" value="{{ $role->class }}">
                    </div>
                                                        ...
                                                        <div class="card-body">
                                                            ...
                                                            @foreach(array_filter(mb_split(";",$i->option)) as $s)
                                                                <div class="custom-control custom-checkbox custom-control-inline">
                                                                    <input type="checkbox" class="custom-control-input" id="{{ $i->name.$loop->iteration }}" name="functions[{{ $i->name }}][]" value="{{ $s }}" {{ strstr($role->function,$i->name.'.'.$s) || $role->function == '777' ? "checked":"" }}>
                                ...
                                <div class="col-12 col-sm-12 col-md-4 pl-5 pb-3">
                                    ...
                                    @foreach($c->curricula as $curricula)
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input" id="class_course{{ $curricula->id }}" name="courses[class_level_{{ $c->id }}][]" value="{{ $curricula->id }}" {{ strpos($role->course,'class_level_'.$c->id.'.'.$curricula->id) === false || $role->function == '777' ? "":"checked" }}>
